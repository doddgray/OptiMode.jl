...
# using .DFTK: LOBPCG
# for now DFTK is just too heavy a dependence to justify for using one sover routine, so I'm copying their code into this file:
# see [here](https://github.com/JuliaMolSim/DFTK.jl/blob/master/src/eigen/lobpcg_hyper_impl.jl)
# I'll have to modify it remove their custom timing macros, etc.
using BlockArrays
export LOBPCG, DFTK_LOBPCG
...
mutable struct DFTK_LOBPCG{L<:AbstractLogger} <: AbstractEigensolver{L}
    logger::L
end
...
############################################################################################################################################
###
###		The code below was copy/pasted from https://github.com/JuliaMolSim/DFTK.jl/blob/master/src/eigen/lobpcg_hyper_impl.jl
###		as a temporary means of avoiding dependancy on DFTK.jl, a very large Density Functional Theory package. I'll figure out how
###		to fix this soon. Their `LOBPCG` ("Locally-Optimal Block-Conjugate Gradient") partial-eigensolver implementation is the
###		the most robust I've found in pure-Julia form, so I like to fall back on it when the more-crash-proned but faster
###		implementation in IterativeSolvers.jl fails. I find this happens near avoided crossings, where I suspect the matrices
###		used in the (Choelsky-based?) subspace orthogonalization routines become ill-conditioned, singular or some such
###		linear algebra thing that leads to a divide-by-zero. This solver has been rock-solid in my experience.		
###
############################################################################################################################################
############################################################################################################################################
############################################################################################################################################

############################ from https://github.com/JuliaMolSim/DFTK.jl/blob/master/src/eigen/preconditioners.jl
