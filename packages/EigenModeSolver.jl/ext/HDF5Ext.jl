"""
    HDF5Ext (EigenModeSolver)

HDF5-based logging extension for EigenModeSolver.jl.

Provides `HDF5Logger` and `hdf5_logger` for structured logging of solver
diagnostics into HDF5 files. Only available when HDF5.jl is loaded.

Based on Tamas Papp's HDF5Logging.jl:
